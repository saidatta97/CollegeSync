      ...
      <div class="container-fluid">
        <div class="panel panel-danger col-sm-8 col-xs-12 col-sm-offset-2 ">
          ...
          <div class="col-sm-4  col-xs-6 col-xs-offset-1  col-sm-offset-4">
            ...
            <input type="text" class="form-control" name="username" value="<?php echo$row[9]; ?>" disabled>
          </div>
        </div>
        <br>
        <div class="row">
...
$user=$_SESSION['user'];
  ...
  $username=$row[9];
  $old=base64_encode($_POST['opwd']);
  $new=base64_encode($_POST['npwd']);
$pwd=$row[10];
if($pwd==$old)
{
   
  $query=mysql_query("update col_registration set REG_PWD='$new' where REG_USERNAME='$username'");
  if($query)
  {
